...
const char* Actor::TypeNames[(int)Actor::EActorType::Max] = {
	"Actor", "BallActor", "FollowActor", "PlaneActor", "TargetActor",
};

Actor::Actor(Game* game)
	: mState(EState::EActive), mPosition(Vector3::Zero),
	mRotation(Quaternion::Identity), mScale(1.0f), mGame(game),
...
// virtual function. not implement yet
...
void Actor::LoadProperties(const rapidjson::Value& inObj) {
	...
	if (JsonHelper::GetString(inObj, "state", state)) {
		if (state == "active") {
			SetState(EState::EActive);
		}
		else if (state == "paused") {
			SetState(EState::EPaused);
		}
		else if (state == "dead") {
