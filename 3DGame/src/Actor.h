...
public:
	...
	static const char* TypeNames[(int)EActorType::Max];

	enum class EState { EActive, EPaused, EDead };

	Actor(class Game* game); // dependency injection -
							 // Actor can access in-game functions.
	virtual ~Actor();

	// only UpdateActor can be overrided.
	...
	Component* GetComponentOfType(Component::EComponentType type) {
	...
	virtual EActorType GetType() const { return EActorType::TActor; }
