...
Renderer::Renderer(Game* game)
	...
	mSkinnedShader(nullptr), mMirrorBuffer(0), mMirrorTexture(nullptr),
	mGBuffer(nullptr), mGGlobalShader(nullptr), mGPointLightShader(nullptr) {}
...
bool Renderer::Initialize(float screenWidth, float screenHeight) {
	...
	mWindow =
		SDL_CreateWindow("Game Programming in C++ (Chapter 14)", 100, 100,
...
bool Renderer::LoadShaders() {
	...
	mProjection = Matrix4::CreatePerspectiveFOV(
		Math::ToRadians(70.0f), mScreenWidth, mScreenHeight, 10.0f, 10000.0f);
	...
	Matrix4 gbufferWorld =
		Matrix4::CreateScale(mScreenWidth, -mScreenHeight, 1.0f);
...
void Renderer::CreateSpriteVerts() {
	...
	mSpriteVerts =
		new VertexArray(vertices, 4, VertexArray::EVertexLayout::PosNormTex, indices, 6);
}
