...
namespace {
	...
	struct MeshBinHeader {
		...
		// Vertex layout type
		VertexArray::EVertexLayout mLayout = VertexArray::EVertexLayout::PosNormTex;
...
Mesh::Mesh()
	: mBox(Vector3::Infinity, Vector3::NegInfinity), mVertexArray(nullptr),
...
bool Mesh::Load(const std::string& fileName, Renderer* renderer) {
	...
	VertexArray::EVertexLayout layout = VertexArray::EVertexLayout::PosNormTex;
	size_t vertSize = 8;

	std::string vertexFormat = doc["vertexformat"].GetString();
	if (vertexFormat == "PosNormSkinTex") {
		layout = VertexArray::EVertexLayout::PosNormSkinTex;
	...
	for (rapidjson::SizeType i = 0; i < vertsJson.Size(); i++) {
		...
		if (layout == VertexArray::EVertexLayout::PosNormTex) {
			Vertex v;
			// Add the floats
	...
	mVertexArray =
		new VertexArray(vertices.data(), numVerts, layout, indices.data(),
			static_cast<unsigned>(indices.size()));

	// Save the binary mesh
	SaveBinary(fileName + ".bin", vertices.data(), numVerts, layout,
...
void Mesh::SaveBinary(const std::string& fileName, const void* verts,
	uint32_t numVerts, VertexArray::EVertexLayout layout,
...
bool Mesh::LoadBinary(const std::string& fileName, Renderer* renderer) {
	...
	if (inFile.is_open()) {
		...
		mVertexArray = new VertexArray(verts, header.mNumVerts, header.mLayout,
			indices, header.mNumIndices);
