...
public:
	...
	enum class EGameState : short { EGameplay, EPaused, EQuit };

	EGameState GetState() const { return mGameState; }
	void SetState(EGameState state) { mGameState = state; }
...
private:
	void ProcessInput();
	void HandleKeyPress(int key); // some op depend on pressed key.
	void UpdateGame();
	void GenerateOutput();
	void LoadData();
	void UnloadData();
