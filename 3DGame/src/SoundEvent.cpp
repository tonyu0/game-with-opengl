...
void SoundEvent::Stop(bool allowFadeOut /* true */) {
  auto event = mSystem ? mSystem->GetEventInstance(mID) : nullptr;
  if (event) {
    FMOD_STUDIO_STOP_MODE mode = allowFadeOut ? FMOD_STUDIO_STOP_ALLOWFADEOUT
...
namespace {
// Direction of coordinates are different between FMOD and this game.
...
// world trans matrix to FMOD 3D attr.
// イベントの位置と方向を更新するためにアクターのワールド変換を渡す処理が単純化される。
// 行列がワールド空間にあるので、逆行列にする必要はない。 →ここがよくわからな
