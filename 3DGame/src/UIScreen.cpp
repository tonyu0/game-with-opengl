...
UIScreen::UIScreen(Game* game)
	: mGame(game), mTitle(nullptr), mBackground(nullptr),
	mTitlePos(0.0f, 300.0f), mNextButtonPos(0.0f, 200.0f),
	mBGPos(0.0f, 250.0f), mState(EUIState::EActive) {
	// Add to UI Stack
	mGame->PushUI(this);
...
void UIScreen::Close() { mState = EUIState::EClosing; }
...
void UIScreen::DrawTexture(class Shader* shader, class Texture* texture,
	...
	Matrix4 scaleMat = Matrix4::CreateScale(
		static_cast<float>(texture->GetWidth()) * scale, yScale, 1.0f);
