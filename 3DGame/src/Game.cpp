...
// process order : rendering -> sound
Game::Game()
	: mRenderer(nullptr), mAudioSystem(nullptr), mPhysWorld(nullptr),
	mGameState(EGameState::EGameplay), mUpdatingActors(false) {}
...
void Game::RunLoop() {
	while (mGameState != EGameState::EQuit) {
		ProcessInput();
		UpdateGame();
		GenerateOutput();
	}
...
void Game::ProcessInput() {
	...
	while (SDL_PollEvent(&event)) {
		switch (event.type) {
		case SDL_QUIT:
			mGameState = EGameState::EQuit;
			break;
			// This fires when a key's initially pressed
		case SDL_KEYDOWN:
			if (!event.key.repeat) {
				if (mGameState == EGameState::EGameplay) {
	...
	if (mGameState == EGameState::EGameplay) {
		for (auto actor : mActors) {
			if (actor->GetState() == Actor::EState::EActive) {
...
void Game::UpdateGame() {
	...
	if (mGameState == EGameState::EGameplay) {
		...
		for (auto actor : mActors) {
			if (actor->GetState() == Actor::EState::EDead) {
	...
	for (auto ui : mUIStack) {
		if (ui->GetState() == UIScreen::EUIState::EActive) {
			ui->Update(deltaTime);
		}
	}
	...
	while (iter != mUIStack.end()) {
		if ((*iter)->GetState() == UIScreen::EUIState::EClosing) {
			delete* iter;
